...
- [ ] **LOC-cap tech debt — split three grandfathered FOCAS modules on next substantive touch (NOT drive-by).** **TRIGGER FIRED for `client.py` 2026-07-28** (D_WEAR fourth-bank read landed) — split deferred per the never-mid-demo clause (live poller session in progress); it is now the FIRST item of the next non-live working session: split along the existing seams (`_FocasLibrary` loader / decoders / `FocasClient`). `shared/focas/client.py` (~1165 LOC — highest-risk module in the tree), `shared/focas/ctypes_defs.py` (~508), `shared/focas/poller.py` (~495) all exceed the 400 cap. Natural seams already exist in `client.py`'s own comment structure (`_FocasLibrary` loader / decoders / `FocasClient`). **Split trigger = the next substantive FOCAS change that lands in each file** (writer path, AG100 per-machine dispatch, param-1013 runtime read) — split for cohesion along those seams *then*, never as a standalone refactor and never mid-demo. Grandfathered per CLAUDE.md until touched.
