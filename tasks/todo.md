...
- [x] Tools list — search + paging over the `{items,total}` envelope (`useTools` TanStack Query hook), compact-spec + assignment badges, links to detail. **Live-verified** the `GET /tools` shape (Decimal→string confirmed, all item keys match the hand-typed `Tool`). Tool **detail** still a placeholder (spec card/assignments/tool-scoped audit = next step).
