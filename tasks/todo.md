...
- [x] Migration `0007_probe_pot_decouple` (2026-07-13): swap `ck_machine_probe_pair_consistent` (probe_pot⟺probe_t_number, assumed fixed probe pot) → `ck_machine_probe_identity_pair` (probe_t_number⟺probe_h_register). The Viper probe **floats** (random-return), so `probe_pot` is now independent/optional and the probe pot is tagged **dynamically** by whichever pot holds T50 (`occupancy.classify_pot`, `PotMap.tsx`). Applied to dev (head 0007), downgrade round-trips. Suite 471 pass; caught a real half-configured probe fixture. See [[viper-machines-tsc]] + lessons.md.
...
- [x] **Operator/seed**: commit a real `shared.machine` Viper row — **DONE 2026-07-13** (dev DB, id `0390689a-0f25-4cb6-9924-a0b859944722`). Panel cross-check with brother revealed the probe **floats** (random-return ATC): T50's pot is not fixed. So the row is `probe_pot=NULL`, `probe_t_number=50`, `probe_h_register=50` (identity lock; R12 is pot-independent), `has_tsc=false` (AP has no TSC; the LG-1000**AG** does — [[viper-machines-tsc]]), `has_toolsetter=true`, `poll_interval_seconds=60`, `random_access`. Required **migration 0007** (decouple `probe_pot` from the probe identity) + **dynamic probe-pot tagging** in `occupancy.py`/`PotMap.tsx`. `manage_users.py` still needed to create the first admin.
...
- [~] **Step-0: productionize the sync poller as a supervised process (R2 deploy shape; the Phase-4 "reads reflect <60s" gate) — PER-MACHINE UNIT DONE + dev-verified 2026-07-10; fleet loop + live gate remain.** Spec `tasks/spec-step0-poller.md`. **Built (the reusable unit):** `shared/focas/service.py` — `FocasService`, a sync, forever-running, self-healing supervisor for ONE machine that wraps the proven `connect → read_snapshot() → persist()` body and **never exits on FOCAS failure** (circuit breaker → close handle → cooldown → reconnect fresh; stale-handle reconnects immediately; a DB/persist error is recorded but does NOT trip the FOCAS breaker). It is synchronous → single-threaded by construction, so running one per machine on its own thread satisfies the §8 thread-affinity + per-machine-breaker rule. Injected clock/sleep/factory/persist → CI-safe. Plus `SingleInstanceLock` (PID file + liveness, auto-reclaims a stale lock) + heartbeat file. `scripts/focas_service.py` — Windows entrypoint (args, DLL dir, line-buffered logging, DSN guard, lockfile, SIGINT/SIGTERM/SIGBREAK). `docs/runbooks/step0-poller-service.md` — Task Scheduler setup + **session-0 DLL-load caveat** w/ logged-on fallback + verify/teardown + optional watchdog. **Key finding:** the "Unreachable · polled Nm ago" fossil needs NO API/UI/schema change — `machines.focas_state` infers `connected` purely from `max(last_polled_at)`. **Verified:** 11 new unit tests (breaker/reconnect/persist-error-isolation/lock/heartbeat; caught + fixed a real heartbeat-aliasing bug → emit an immutable `replace()` snapshot); suite **418 pass/1 skip** w/ dev DSN; ruff+mypy clean (service.py under the gate). **Dev-DB fossil→live flip proven** (labeled mock source + real persist): `connected=False`/no-mirror → 3 cycles → `connected=True` lag 0.03s, offsets/pots/status mirrored → torn down clean (cascade). **DECIDED 2026-07-10 (dbc00per): stay on the current path; document the fleet target, don't build it now** (no future-regression conflict — see `spec-step0-poller.md` §11). Interim shape = **one-process-per-machine** (valid + strongly isolated); the frozen §8.2 fleet loop is an *additive* orchestrator over the same `FocasService`, so nothing shipped needs a rewrite. **REMAINING (tracked follow-ons, built as machines physically arrive — NOT ahead of §7 onboarding gates):** (1) ~~**fleet launcher**~~ **✅ BUILT + CUT OVER 2026-08-06** (`shared/focas/fleet.py` + `scripts/focas_fleet.py`, commit 56db758): concurrency gate PASSED (4 handles × 4 threads, ~1,500 cycles, 0 errors — `reports/fwlib-concurrency-check-20260806.json`); registry-driven (enabled rows + `--also` for soaks), one process now polls all 6 active machines (dev_stack cut over); single Task Scheduler entry ready for the dedicated poller PC. (2) **`monitor-only` read profile** — the ~7 tool-life+cycle-time machines need a LIGHT partial read, not Viper's ~35s full sweep (`persist()` is additive-upsert, so partial snapshots are safe). (3) **cycle-time read path** — NEW FOCAS read (run/part timers), not in any snapshot yet. (4b) **4 × VIPER VT lathes — IN PROGRESS 2026-08-06 (`tasks/spec-vtfleet-onboarding.md`)**: VT-21 (.51, 0i-TD), VT-23A (.52, 0i-TF — the live Lathe 3 renamed to **VT-23B**), VT-25BL (.54, 0i-TD), VT-15L (.55, 0i Mate-TD); gates 0–4+7 done read-only, sweeps clean incl. the 0i-D generation, tool-life licensed on all four; open = panel cross-check sheets (`reports/vtfleet-panel-crosscheck-*.md`), active-T glances, soak, enable. **Fleet is now 10 machines** — the §8.2 fleet launcher (one process per machine × 10) is getting due. (4) **3 × 2026 5-axis dual-spindle lathes — IN PROGRESS 2026-08-05 (`tasks/spec-panther-onboarding.md`)**: the Panther group (JAKE_2100LY .56, JAKE_2100LYS .57, PROD_2100LYS-2 .60, all 0i-TF Plus D6G3) passed gates 0–4 + 7 read-only — registry rows in `enabled=false`, capability sweeps clean (128 regs, all 7 T banks, tool-life licensed), units fleet-verified via new `cnc_rdparam`/`cnc_rdset` binding (`shared/focas/params.py`: INI inch + 1013 IS-B → 0.0001 on all six machines). Remaining: gate 5 (turret/active-tool source — `cnc_rdcommand` candidate, verify like VT), gate 6 (panel cross-check sheets printed in `reports/panther-panel-crosscheck-*.md`, dbc00per at panels), **path-2 discovery on the two TT (sub-spindle) controls** (nothing models path 2 today), gates 8–9 (soak, enable) + dev_stack blocks. (5) ~~live-Viper gate~~ **✅ PASSED 2026-07-10 (read-only, machine actively cutting)** — ran `focas_service.py` vs 10.1.10.58: 35+ cycles / 0 failures / ~36s per cycle; **tracked two real tool changes live** (HEAD/NEXT 85/6→6/30→30/6); hard-kill→restart proved **reconnect (0.2s) + stale-lock auto-reclaim** (poller-side only, machine untouched); temp row torn down, dev DB clean. Task-Scheduler production install + boot-trigger confirmation **DEFERRED (dbc00per 2026-07-10) to an after-hours window (shop empty)**, ideally on a **dedicated poller host, not this fragile dev/build PC**; the non-reboot parts (`schtasks /Run`, kill→restart) can be validated anytime — see runbook §4.4. **Non-reboot mechanics RE-VALIDATED via the entrypoint 2026-07-13** (read-only vs live Viper, temp machine row, torn down clean): heartbeat JSON (`state:healthy`, `cycles_ok` climbing, `last_cycle_seconds`≈35s), cadence guardrail warning fired (interval 5 < cycle 36), hard-kill→restart, and **stale-lock auto-reclaim** (dead pid 62240 → new pid 19360). `schtasks /Run` itself is part of the gated Task-Scheduler install (not created on the build PC); exact prod command is prepped (runbook §4.2, only `--machine-uuid` was pending — now `0390689a-…`). **Config finding (lessons.md):** `poll_interval_seconds` must be ≥ measured ~36s cycle time or `connected` flaps False — set **60** for Viper, not 10. (6) permanent `shared.machine` Viper row still gated on a verified `probe_pot` (and seed `poll_interval_seconds=60`, per the finding above). Watchdog documented-not-shipped (add only if a live soak shows a hang).
- [~] **LOC-cap tech debt — `client.py` SPLIT DONE 2026-07-30** (branch
  `claude/client-split`): ~1165 LOC → 4 modules along the documented seams —
  `loader.py` (272: DLL load + signatures), `decoders.py` (386: verified
  binding constants + pure decoders), `client_reads.py` (306: bulk reads as
  functions taking the client, the `lathe.py` idiom), `client.py` (400:
  FocasClient lifecycle/identity/status/PMC + snapshot orchestration,
  re-exporting the whole surface so every existing import resolves
  unchanged). Zero behavior change: full suite identical to pre-split
  baseline, ruff+mypy clean. **Remaining grandfathered:**
  `shared/focas/ctypes_defs.py` (~508) + `shared/focas/poller.py` (~495) —
  split on next substantive touch (writer path, per-machine dispatch), never
  as a standalone refactor.
- [ ] **Phase 5/6 write-path — first-PR shape is LOCKED (policy → code before any `cnc_wrtofs`).** When the write path starts, the opening PR is, in order: (1) mode-lockout helper (verify NOT running / NOT AUTO) + tests; (2) the double-wall API contract (the ask + the `WRITE_APPROVAL_PASSWORD` entry) + tests that PROVE the write is blocked unless both walls AND mode-lockout hold; (3) a mock-only write surface (`tooling/focas/mock.py`). The actual `cnc_wrtofs` binding lands LAST, behind the gate — never first. Enforces the CLAUDE.md HARD GATE in code, not just docs. dbc00per sign-off required before merge (Stop Conditions). Mill-scoped first; lathe write surface is a later profile allow-list (`docs/10` §10). **Full plan drafted 2026-07-10: `tasks/spec-phase5-write-path.md`** (DRAFT for review — grounds the binding: `cnc_wrtofs`/`cnc_wrtofsr` confirmed in `Fwlib64.h`; the `offset_write_request` table already models request→confirm→execute→verify; offset math + type-code map + param-1013 check from lessons.md; D1–D5 open decisions flagged). **PR-1 + PR-2 BUILT 2026-07-10 (mock-only, no `cnc_wrtofs`, no live write):** `apps/tooling/api/services/write_safety.py` — `mode_lockout_ok` (not running + safe mode, fail-closed on MEM/AUTO/UNKNOWN) + `authorize_write` composing probe-lock (R12) → mode-lockout → the ask (`approved`) → the entry (`WRITE_APPROVAL_PASSWORD`, constant-time, **fail-closed when unset**). New `Settings.write_approval_password` (env-only, `.env.example` documented, never committed). **22 gate-blocks tests** prove a write is refused on every wall + correct precedence; suite 376 pass, ruff+mypy clean (67 files). NB the mode-lockout safe-mode set is a conservative default that needs operator/brother validation before the first live write (spec D5). **D1 + D5 DECIDED 2026-07-10 (pm-4):** D1 = **batch allowed** (one ask + one password entry covers multiple registers; per-register ack retained per R6; drift-abort per register); D5 = first live write is a **single supervised H_GEOM register + panel cross-check** (subset of the batch capability). D2–D4 stay at recommended defaults (unconfirmed); D3 derived to **per-confirmation** password cadence given D1 batch. See `spec-phase5-write-path.md` §7. **REMAINING (still sign-off-gated): PR-3 mock-only write surface + `offset_write_request` lifecycle (batch-aware per D1); PR-4 real `cnc_wrtofs` binding + offset math + param-1013 check; then D5 first supervised live write.**
